...
public class LoginControllerFX {
    ...
    private void register() {
        int newUser = realController.LoginController.createNewUser(username.getText(), password.getText());
        if (newUser == 0)
        {
    ...
    private void login() throws IOException {
        int i = realController.LoginController.loginUser(username.getText(), password.getText());
        if (i == 0)
